...
const fetchCharacterRequestSuccess = ( state, action ) => {
  return update( state, {
    ...
    data_backup: { $set: action.response.results || []},
    isLoading: { $set: false },
    message: { $set: "Success" },
  })
}
...
const doSearch = ( state, action ) => {
    let oldData = state.data_backup
    let newData = []
    let sText = action.payload.trim().toLowerCase();
    if( sText != ""){
        oldData.map((i) => {
